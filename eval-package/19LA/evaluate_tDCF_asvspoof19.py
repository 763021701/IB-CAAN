...
def compute_eer_and_tdcf(cm_score_file, path_to_database):
    asv_score_file = os.path.join(path_to_database, 'ASVspoof2019.LA.asv.eval.gi.trl.scores.txt')
    ...
    cm_keys = cm_data[:, 2]
    cm_scores = cm_data[:, 3].astype(np.float32)
...
def test_individual_attacks(cm_score_file, path_to_database):
    asv_score_file = os.path.join(path_to_database, 'ASVspoof2019.LA.asv.eval.gi.trl.scores.txt')
