...
from data_utils import (
    ...
    Dataset_ASVspoof2019_train_for_SWA,
    Dataset_ASVspoof2019_dev,
    Dataset_ASVspoof2021_eval,
    Dataset_in_the_wild_eval,
    ATTACK_STR, ATTACK_2_INT
)
from utils import (
    seed_worker,
    set_seed,
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.eval:
        ...
        trainer.produce_evaluation_file(eval_loader, eval_score_path)
        sys.exit(0)
    ...
    for epoch in range(config["num_epochs"]):
        ...
        val_result = trainer.validation_one_epoch(dev_loader, metric_path, database_path, 'asvspoof2019')
...
def get_trainer(model, device, optim_config, config):
    ...
    elif config["trainer"] == "dann":
        from trainers.DANN_Trainer import DANN_Trainer
        return DANN_Trainer(model, device, optim_config, config)
...
def get_loader(
    ...
    trn_database_path = database_path / "ASVspoof2019_LA" / "ASVspoof2019_LA_train/flac/"
    dev_database_path = database_path / "ASVspoof2019_LA" / "ASVspoof2019_LA_dev/flac/"

    trn_list_path = (database_path / "ASVspoof2019_LA" /
                     "ASVspoof2019_LA_cm_protocols/ASVspoof2019.LA.cm.train.trn.txt")
    dev_trial_path = (database_path / "ASVspoof2019_LA" /
                      "ASVspoof2019_LA_cm_protocols/ASVspoof2019.LA.cm.dev.trl.txt")

    d_label_trn, d_speaker_trn, d_attack_trn, file_train, attack_label_trn = genSpoof_list(
    ...
    train_set_swa = Dataset_ASVspoof2019_train_for_SWA(
        config["data_aug_config"],
        list_IDs=file_train,
        labels=d_label_trn,
        base_dir=trn_database_path,
    ...
    d_label_dev, d_speaker_dev, d_attack_dev, file_dev = genSpoof_list(
        dir_meta=dev_trial_path, is_train=False, is_eval=False)
    print("no. validation files:", len(file_dev))

    dev_set = Dataset_ASVspoof2019_dev(
        config["data_aug_config"],
        list_IDs=file_dev,
        labels=d_label_dev,
        base_dir=dev_database_path,
        speakers=d_speaker_dev
    )
...
def get_eval_loader(
    ...
    file_eval = []
    if config["track"] == "19LA":
        eval_database_path = (database_path
                              / "ASVspoof2019_LA"
                              / "ASVspoof2019_LA_eval"
                              / "flac")
        eval_list_path = (database_path
                          / "ASVspoof2019_LA"
                          / "ASVspoof2019_LA_cm_protocols"
                          / "ASVspoof2019.LA.cm.eval.trl.txt")
        _, _, _, file_eval = genSpoof_list(dir_meta=eval_list_path,
                                        is_train=False,
                                        is_eval=False)

    elif config["track"] == "21LA" or config["track"] == "21DF":
        _track = "LA" if config["track"] == "21LA" else "DF"
        eval_database_path = (database_path
                              / "ASVspoof2021_{}_eval".format(_track)
                              / "flac")
        eval_list_path = (database_path
                          / "ASVspoof2021_{}_eval".format(_track)
                          / "ASVspoof2021_{}_cm_protocols".format(_track)
                          / "ASVspoof2021.{}.cm.eval.trl.txt".format(_track))
        file_eval = genSpoof_list(dir_meta=eval_list_path,
                                  is_train=False,
                                  is_eval=True)

    elif config["track"] == "ITW":
        eval_database_path = (database_path
                              / "release_in_the_wild")
        eval_list_path = (database_path
                          / "release_in_the_wild"
                          / "in_the_wild.eval.trl.txt")
        file_eval = genSpoof_list(dir_meta=eval_list_path,
                                  is_train=False,
                                  is_eval=True)
    else:
    ...
    if config["track"] == "19LA" or config["track"] == "21LA" or config["track"] == "21DF":
