...
from .network_utils import Classifier
...
class RawBMamba_Featurizer(nn.Module):
    def __init__(self, d_args):
        super().__init__()
        self.device=d_args["device"]
        self.hidden_dim = d_args["emb_dim"]
        self.n_layer = d_args["n_layer"]

        self.mamba_config = MambaConfig(
            d_model=self.hidden_dim, n_layer=self.n_layer)
    ...
    def forward(self, x):
        x = x.unsqueeze(1)
        _, last_hidden = self.model(x)
        return last_hidden


class RawBMamba_Classifier(nn.Module):
    def __init__(self, d_args):
        super().__init__()
        self.d_args = d_args
        self.hidden_dim = d_args["cls_dim"]
        self.out_dim = d_args["nb_classes"]
...
class Model(nn.Module):
    def __init__(self, d_args):
        super().__init__()
        self.Featurizer = RawBMamba_Featurizer(d_args)
        self.Classifier = RawBMamba_Classifier(d_args)
